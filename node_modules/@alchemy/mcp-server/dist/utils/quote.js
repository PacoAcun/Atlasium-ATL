import { ethers } from 'ethers';
import { CurrentConfig } from '../libs/config.js';
import { computePoolAddress } from '@uniswap/v3-sdk';
import Quoter from '@uniswap/v3-periphery/artifacts/contracts/lens/QuoterV2.sol/QuoterV2.json' with { type: "json" };
import IUniswapV3PoolABI from '@uniswap/v3-core/artifacts/contracts/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json' with { type: "json" };
import { POOL_FACTORY_CONTRACT_ADDRESS, QUOTER_CONTRACT_ADDRESS, } from '../libs/constants.js';
import { getProvider } from '../libs/providers.js';
import { toReadableAmount, fromReadableAmount } from '../libs/conversion.js';
export async function quote() {
    const quoterContract = new ethers.Contract(QUOTER_CONTRACT_ADDRESS, Quoter.abi, getProvider());
    console.error('quoterContract', quoterContract);
    const poolConstants = await getPoolConstants();
    try {
        const quotedAmountOut = await quoterContract.callStatic.quoteExactInputSingle({
            tokenIn: poolConstants.token0,
            tokenOut: poolConstants.token1,
            fee: poolConstants.fee,
            amountIn: fromReadableAmount(CurrentConfig.tokens.amountIn, CurrentConfig.tokens.in.decimals).toString(),
            sqrtPriceLimitX96: 0,
        });
        console.log({ quotedAmountOut });
        return toReadableAmount(quotedAmountOut.amountOut, CurrentConfig.tokens.out.decimals);
    }
    catch (error) {
        console.error(error);
        return '0';
    }
}
async function getPoolConstants() {
    const currentPoolAddress = computePoolAddress({
        factoryAddress: POOL_FACTORY_CONTRACT_ADDRESS,
        tokenA: CurrentConfig.tokens.in,
        tokenB: CurrentConfig.tokens.out,
        fee: CurrentConfig.tokens.poolFee,
    });
    const poolContract = new ethers.Contract(currentPoolAddress, IUniswapV3PoolABI.abi, getProvider());
    const [token0, token1, fee] = await Promise.all([
        poolContract.token0(),
        poolContract.token1(),
        poolContract.fee(),
    ]);
    return {
        token0,
        token1,
        fee,
    };
}
