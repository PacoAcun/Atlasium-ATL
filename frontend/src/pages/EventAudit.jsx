    ...
    <LayoutResponsive>
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-white">Auditoría: {currentEvent.name}</h1>
            <p className="text-gray-400 text-sm font-mono">{currentEvent.wallet_address}</p>
          </div>
                  ...
                  <div key={tx.hash} className="p-4 flex items-center justify-between hover:bg-neutral-800/50 transition">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 font-mono">
                          {isIncoming ? `De: ${tx.from.slice(0, 6)}...${tx.from.slice(-4)}` : `A: ${tx.to.slice(0, 6)}...${tx.to.slice(-4)}`}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="flex items-center justify-end gap-1 text-xs text-gray-500 mt-1">
                        <FiClock size={10} />
                        <span>{new Date(tx.metadata.blockTimestamp).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                );
